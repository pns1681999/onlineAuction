...
module.exports={
    ...
    countAutionByBidder: async bidderId => {
        const rows = await db.load(`SELECT count(*) as total FROM (SELECT a.IdSanPham FROM daugia a,sanpham b where a.IdNguoiDung=${bidderId} and a.IdSanPham=b.IdSanPham and b.NgayHetHan>SYSDATE() group by a.IdSanPham )c`)
        return rows[0].total;
      },
    pageAutionByBidder: (bidderId, offset) => db.load(`SELECT * FROM (SELECT a.IdSanPham,b.TenSanPham,b.GiaHienTai,b.NgayHetHan, b.IdNguoiThang FROM daugia a, sanpham b where a.IdNguoiDung=${bidderId} and a.IdSanPham=b.IdSanPham and b.NgayHetHan>SYSDATE() group by a.IdSanPham
